...
data class LineGraphValues(
    ...
    private val yMax: Float
        get() {
            return listOfData.maxByOrNull { it.yValue }?.yValue ?: 0f
        }

    private val upperYValue: Float = yMax * (DATASET_MAX_VALUE_PADDING)

    val yLabelValues: List<String>
        get() {
            val yValues = mutableListOf<String>()
            repeat(NUMBER_OF_Y_LABELS) {
                val value = upperYValue * ((1f / NUMBER_OF_Y_LABELS) * (NUMBER_OF_Y_LABELS - it))
                yValues.add(getFormatedNumber(value.toLong()))
            }
    ...
    fun getDataPoints(quadrantRect: Rect): List<Pair<Offset, DataPoint>> {
        ...
        listOfData.forEachIndexed { index, point ->
            ...
            listOfDataPoints.add(Pair(Offset(valueBetweenPoints + quadrantRect.left, yPoint), point))
        }
